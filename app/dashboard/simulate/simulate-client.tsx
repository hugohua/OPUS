...
export function SimulateClient({ matrix }: { matrix: TrainingMatrix }) {
...
function MatrixScenarioCard({
    ...
    return (
        <SimulateScenarioCard
            title={entry.title}
            desc={entry.subtitle}
            tag={entry.statusLabel ?? entry.tag}
            icon={Icon}
            href={href}
        ...
        >
            <div className="flex justify-between items-start w-full mb-3">
                ...
                <span className={cn(
                    ...
                    entry.availability === "ready" ? "bg-amber-500" : "bg-zinc-200 dark:bg-zinc-800 group-hover:bg-amber-500"
                )} />
            </div>
