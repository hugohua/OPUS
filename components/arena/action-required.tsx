    ...
    if (nodes.length === 0) {
        ...
        return (
            <section className="mt-8">
                ...
                <div className="bg-surface dark:bg-zinc-900 border border-border rounded-xl p-6 shadow-sm text-center flex flex-col items-center">
        ...
        <section className="mt-8">
            ...
            <div className="space-y-3">
                {nodes.map((node, index) => {
                    ...
                    if (isWorst) {
                        return (
                            <div key={node.id} className="bg-surface dark:bg-zinc-900 border border-rose-200 dark:border-rose-900/50 rounded-xl p-4 shadow-sm relative overflow-hidden group">
                                ...
                                <div className="flex justify-between items-start mb-3 pl-2">
                                    ...
                                    <div className="flex flex-col items-end shrink-0">
                                        ...
                                        <span className="text-[9px] font-mono text-zinc-500 dark:text-zinc-400">MASTERY</span>
                                    </div>
                                </div>
                    ...
                    return (
                        <div key={node.id} className="bg-surface dark:bg-zinc-900 border border-border rounded-xl p-4 shadow-sm relative pl-5 opacity-80 hover:opacity-100 transition-opacity">
                ...
                {nodes.length > 0 && Array.from({ length: Math.max(0, 3 - nodes.length) }).map((_, idx) => (
                    <div key={`filler-${idx}`} className="bg-surface dark:bg-zinc-900 border border-dashed border-border rounded-xl p-4 shadow-sm relative pl-5 opacity-50 flex items-center justify-center">
                        <span className="text-xs font-mono text-zinc-400 dark:text-zinc-600">-- Level Passed --</span>
                    </div>
                ))}
            </div>
        </section>
