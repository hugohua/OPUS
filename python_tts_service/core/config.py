...
class Config:
    ...
    # 阿里云 DashScope 配置
    # 必须使用 TTS_API_KEY，严格隔离
    OPENAI_API_KEY: str = os.getenv("TTS_API_KEY")
    OPENAI_BASE_URL: str = os.getenv("TTS_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")
    
    # TTS 模型配置
    # TTS 模型配置
    # 允许通过环境变量覆盖
    TTS_MODEL: str = os.getenv("TTS_MODEL", "qwen3-tts-flash")
    DEFAULT_VOICE: str = os.getenv("TTS_DEFAULT_VOICE", "Cherry")
    DEFAULT_LANGUAGE: str = "English"
    DEFAULT_SPEED: float = float(os.getenv("TTS_DEFAULT_SPEED", "1.0"))
    ...
    # 优先读取环境变量 CACHE_DIR
    _ENV_CACHE_DIR = os.getenv("CACHE_DIR")
    if _ENV_CACHE_DIR:
        CACHE_DIR: Path = Path(_ENV_CACHE_DIR)
    else:
        # 默认回退逻辑
