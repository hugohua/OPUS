...
为了防止缓存耗尽（Cache Miss）与数据库并发雪崩（N+1 Problem），竞技场实现了专用的 `arena-fallback.ts` 隔离层：
- **前置批查询 (Batch Pre-load)**：在 `get-next-drill.ts` 处理提取时，主动过滤得到缺货的竞技场目标单词，合并为一个 `findMany(in:)` 语句极速取回 Seed 数据。
- **纯函数组装**：调用 `buildArenaFallbackDrill(candidate, mode, seed)`，若命中 seed 降级为原题组装，若极端情况下数据库仍空，则执行**极限正则兜底**（运用 `\b` 边界正则基于词根变化 `s/ed/ing` 生造四个选项），确保无论如何一定有题可做，保证核心功能不断流。
