...
## 2. 竞技场架构概览：Part 6 (长文多空模式)

Part 6 是 Opus 竞技场中最为复杂的沉浸式长篇阅读题。一次考察 4 个 Target Words 且共用一篇商务场景连贯长文。区别于典型小题的同步/实时组装模式，它采用了**异步预生成 + 瞬时提单兜底**的架构：

### 2.1 生产者-消费者解耦模型 (The Dual-Track Architecture)
- **前端极速消费 (O(1))**: `actions/part6-queue.ts` 不再包含任何 `generateObject` 阻塞调用。它仅负责以 $O(1)$ 的速度从 Redis `ARENA_PART6` 缓存池里 `pop` 取出预组装好的数据模型。以零等待时间返回给客户端。
- **后端深耕生产**: 耗时的模型调用、多词汇挑选与故事构建均被剥离至独立的 `workers/handlers/part6-handler.ts` 进行无感并发处理。这隔离了长文本对生成耗时的敏感交互卡顿（吞吐率绑架）。

### 2.2 防断笼机制 (Deterministic Local Fallback)
由于 Part 6 每次消耗的 Token 量巨大且容易触碰第三方 API 的超时熔断器（Timeout）或是输出不可控格式失败，系统专门在两个层面配置了兜底：
1. **服务端取件兜底 (`actions/part6-queue.ts`)**: 在 Redis 被挂空（Cache Miss）时，立即截断响应时间（不再等待 Worker 紧急介入），直接调用本地模版 (`buildArenaPart6FallbackDrill`) 返回一套基于"IT 服务维护"的静态占位兜底题。
2. **生成端安全网 (`workers/handlers/part6-handler.ts`)**: 若后台 LLM 大模型长文推导抛错，系统会静默捕捉并在 Redis 里注水一条同样的本地降级题目，确保下游源源不断有题分发。

---

## 3. 端到端新增全新题型 (Session Mode) Checklist
