/**
 * Session Drill 批量获取共享核心
 * 
 * 功能：
 *   获取下一批 Drill 卡片，供 Web Server Action、H5 API 和 iOS Demo API 共用。
...
import { getEnginePreferencesByUserId } from '@/lib/backend-core/settings/preferences';

const log = createLogger('backend-core:session:batch');
...
// --- Main Core ---
export async function getSessionDrillBatchForUser(
    ...
    try {
        ...
        log.error({ error }, 'getSessionDrillBatchForUser failed');
        return {
            status: 'error',
